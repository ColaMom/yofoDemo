...
- (nullable UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    //    高度
    CGFloat height = 0;
    if ([self.delegate respondsToSelector:@selector(FJWaterFlowlayout:collectionView:heightForWidth:atIndexPath:)]) {
        height = [self.delegate FJWaterFlowlayout:self collectionView:self.collectionView heightForWidth:width atIndexPath:indexPath];
    }
...
- (nullable UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([UICollectionElementKindSectionHeader isEqualToString:elementKind]) {
        ...
        if ([self.delegate respondsToSelector:@selector(FJWaterFlowlayout:collectionView:referenceSizeForHeaderInSection:)]) {
            size = [self.delegate FJWaterFlowlayout:self collectionView:self.collectionView referenceSizeForHeaderInSection:indexPath.section];
        }
//        CGFloat x = self.sectionInset.left;
    ...
    //footer
        ...
        //size
        CGSize size = CGSizeZero;
        if ([self.delegate respondsToSelector:@selector(FJWaterFlowlayout:collectionView:referenceSizeForFooterInSection:)]) {
            size = [self.delegate FJWaterFlowlayout:self collectionView:self.collectionView referenceSizeForFooterInSection:indexPath.section];
        }
//        CGFloat x = self.sectionInset.left;
        CGFloat x = 0;
