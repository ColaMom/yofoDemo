...
- (void)setupUI {
    ...
    if (![CLStyleConfig sharedConfig].tagCornerRadius) {
        [CLStyleConfig sharedConfig].tagCornerRadius = _tagHeight * 0.5;
    }
    _inputField.layer.cornerRadius = [CLStyleConfig sharedConfig].tagCornerRadius;
...
- (void)addTagWithTag:(NSString *)text {
    if (![self.tagCache objectForKey:text]) {
        ...
        [tagBtn setTitleColor:[CLStyleConfig sharedConfig].tagSelectedTextColor forState:UIControlStateNormal];
        tagBtn.backgroundColor = [CLStyleConfig sharedConfig].tagSelectedBackgroundColor;
        [tagBtn rounded:[CLStyleConfig sharedConfig].tagCornerRadius width:[CLStyleConfig sharedConfig].tagBorderWidth color:[CLStyleConfig sharedConfig].tagSelectedTextColor];
...
- (void)removeTagWithTag:(NSString *)tag {
    ...
    if (tagbtn) {
        ...
        if (self.tagBtnArrayM.count>index) {
...
- (void)setSelectedTags:(NSArray<RHFArticleTagModel *> *)selectedTags{
    _selectedTags = selectedTags;
    for (RHFArticleTagModel *tagModel in selectedTags) {
        [self addTagWithTag:tagModel.val];
    }
}
